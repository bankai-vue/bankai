...
- [x] `BankaiInputPassword` _(universal)_ — own native `<input>` (reuses the `bankai-input` class) whose `type` flips `password`↔`text`. String default `v-model`; the reveal state is a named `v-model:revealed` that drives the `type` even with the built-in button off. **Reveal button on by default** (opt out with `:toggle="false"` for a bare `<input>`) — a **real, focusable, tab-order** control (not `aria-hidden` like the number steppers) whose accessible name/label swaps between the localized `inputPassword.show`/`hide` messages (per-instance `showLabel`/`hideLabel`; `#toggle` slot for an icon). Exposes `el`/`focus`/`blur`/`select`. Landing this 3rd input control extracted the shared `id`/ref/`focus`/`blur`/`select` wiring into an internal `useBankaiInput` composable, now adopted by all three. **OWED:** the real-screen-reader pass (NVDA/JAWS/VoiceOver) on the reveal toggle
- [ ] `BankaiCheckbox`
