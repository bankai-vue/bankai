...
| `BankaiFlex` · `Grid` · `Stack` · `Group`                                                    | `<div>` (polymorphic via `as`) — inline `display:flex`/`grid` from props, no CSS file (Stack/Group are Flex presets)                                     |
...
Layout utilities — prop-driven inline styles, agnostic (no theme CSS needed); polymorphic `as` (default `<div>`). The composable replacement for Vuetify `VRow`/`VCol`:

- [ ] `BankaiFlex` — flexbox helper; `direction`/`align`/`justify`/`gap`/`wrap`/`inline` props → inline `display:flex`
- [ ] `BankaiGrid` — CSS-grid helper; `columns`/`rows`/`gap`/`areas` props → inline `display:grid` (2D layouts)
