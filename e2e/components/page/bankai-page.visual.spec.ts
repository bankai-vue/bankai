...
// Visual regression for BankaiPage. The component ships no CSS — the theme only fills its min block size
// (no visible paint of its own); the fixture bounds a frame and outlines the page box so the baseline
// captures the filled region. Baselines are per-OS (Playwright suffixes them with the platform) and are
// generated in CI via the "Update Visual Snapshots" workflow, not locally.

for (const colorScheme of ['light', 'dark'] as const) {
  test(`BankaiPage — ${colorScheme}`, { tag: '@visual' }, async ({ page }, testInfo) => {
    ...
    const fixture = page.getByTestId('page-fixture');

    // Guard: confirm the fixture actually rendered before snapshotting.
    await expect(fixture).toBeVisible();

    const snapshot = `bankai-page-${colorScheme}.png`;
