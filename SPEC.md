...
- _Prop-resolver helpers (share-or-hoist):_ logic that coerces a prop to a CSS value (a spacing step → `--bankai-space-*`, a track count → `repeat(…)`, an area array → quoted rows) is a **pure, stateless helper**, never inlined in the render and never left inside `<script setup>` where it re-allocates per instance. Decide its home per helper: if another shipped-or-planned component reuses the same coercion, **share** it in `src/internal/` (with an options param so a divergent token family can override defaults instead of forking) — promoted to a public composable only once that API is planned and documented; otherwise **hoist** it to the SFC's module-scope `<script lang="ts">` block so it's allocated once. Shipped example: `resolveGap` is shared by `BankaiFlex` + `BankaiGrid` (and the Stack/Group Flex presets) in `internal/spacing.ts`, while `resolveTracks`/`resolveAreas` are Grid-only and live at `BankaiGrid`'s module scope.
