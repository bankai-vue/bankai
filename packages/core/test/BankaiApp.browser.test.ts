...
import { BankaiApp } from '../src/index';
...
// Mounts BankaiApp into the real browser DOM and returns the rendered root
// element plus a teardown. Dependency-free (no test-utils).
function mountApp(props: Record<string, unknown> = {}, slot: Slot = 'content'): Mounted {
  const host = document.createElement('div');
  document.body.append(host);
  const renderSlot = typeof slot === 'function' ? slot : (): VNodeChild => slot;
  const app = createApp(() =>
    slot === null ? h(BankaiApp, props) : h(BankaiApp, props, renderSlot),
  );
  ...
  if (!(root instanceof HTMLElement)) {
    throw new Error('BankaiApp did not render an element');
  }
...
// BankaiApp ships no CSS — the theme carries the embedded surface (color-scheme + bg/fg) on its box.
// These unit tests assert the rendered anatomy (the plain `<div>` element, class, `data-part`); the
// surface paint the theme resolves is covered by the e2e/visual tests, which load the theme.
test('renders a plain <div> exposing the app anatomy', () => {
  const { root, teardown } = mountApp();

  expect(root.tagName).toBe('DIV');
  expect(root.classList.contains('bankai-app')).toBe(true);
...
test('renders without slot content', () => {
  const { root, teardown } = mountApp({}, null);
...
test('merges consumer class/style/attributes onto the root', () => {
  const { root, teardown } = mountApp(
    { class: 'my-app', style: { color: 'red' }, 'data-testid': 'app' },
    'content',
  );

  // Consumer class merges with the component's own `bankai-app`.
  expect(root.classList.contains('bankai-app')).toBe(true);
  expect(root.classList.contains('my-app')).toBe(true);
  expect(root.dataset.testid).toBe('app');
...
test('does not let a consumer fallthrough clobber the owned data-part', () => {
  const { root, teardown } = mountApp({ 'data-part': 'evil' }, 'content');
